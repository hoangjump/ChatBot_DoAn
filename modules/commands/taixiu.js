...
module.exports.run = async function({ api, event, args, Threads, Users, Currencies }) {
  try {
    ...
    if (args[0] == 'create' || args[0] == 'new' || args[0] == '-c') {
      ...
      if (!gameThread) return api.sendMessage('⚠ Hiện tại nh��m này chưa có bàn game nào!', threadID, messageID);
      ...
      if (gameThread.author != senderID) return api.sendMessage('⚠ Bạn không phải là người tạo ra bàn game nên không th�� xóa bàn game', threadID, messageID);
      global.taixiuS.delete(threadID);
